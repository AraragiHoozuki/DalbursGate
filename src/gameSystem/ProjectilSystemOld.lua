...
ProjectilMgr.CreateAttackProjectil = function(lu_emitter, u_target, damage_value)
    local settings = Master.DefaultAttackProjectil[GetUnitTypeId(lu_emitter.unit)]

    local x = GetUnitX(lu_emitter.unit)
    local y = GetUnitY(lu_emitter.unit)
    local theta = GetUnitFacing(lu_emitter.unit) * math.degree
...
---@param level number
---@return Projectil
function Projectil:new(o, lu_emitter, x, y, z, yaw, settings, target_unit, target_position, damageSettings, level)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    --settings
...
function Projectil:ctor(obj)
    ...
    local x = obj.x or GetUnitX(obj.emitter.unit)
    local y = obj.y or GetUnitY(obj.emitter.unit)
    local dx
    local dy
    if (obj.target_unit ~= nil) then
        dx = GetUnitX(obj.target_unit) - x
        dy = GetUnitY(obj.target_unit) - y
    elseif obj.target_position ~= nil then
        dx = obj.target_position.x - x
        dy = obj.target_position.y - y
    end
    local angle = math.atan(dy,dx)

    local offsetX = obj.offsetX or settings.offsetX
    local offsetY = obj.offsetY or settings.offsetY
    local offsetZ = obj.offsetZ or settings.offsetZ
    -- calc emit offset
    local theta = angle
    x = offsetY * Cos(theta) - offsetX * Sin(theta) + x
    y = offsetY * Sin(theta) + offsetX * Cos(theta) + y
    local z = obj.z or settings.offsetZ or 0

    return Projectil:new(obj, obj.emitter, x, y, z, angle, settings, obj.target_unit, obj.target_position, obj.damageSettings or {}, obj.level)
end
...
function Projectil:ChangeModel(new_model)
    self.model = new_model
    DestroyEffect(self.bullet)
    self.bullet = AddSpecialEffect(new_model, self.position.x, self.position.y)
